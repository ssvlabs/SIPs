...
**State fold transition table.** A log for an event in this table that cannot be decoded as its ABI event is malformed and changes no record. Other malformed events change no record except where a transition below explicitly happens before validation.

| Event | Accept when | Transition |
| ----- | ----------- | ---------- |
| OperatorAdded | `operator_id` is absent and `publicKey` decodes to `canonical_public_key` under the operator key rule below | Insert Operator with `removed = false`. |
| OperatorRemoved | Operator exists | If an included cluster still references the operator, set `removed = true`; otherwise omit the Operator record. |
| ValidatorAdded | After the owner nonce transition, `validator_public_key` is valid, the `(validator_public_key, owner)` identity is absent, `operatorIds` is nonempty, sorted, unique, has a valid `3f+1` size, every operator exists with `removed = false`, the cluster is absent or present with `liquidated = false`, the share blob has the expected length, and `validator_signature` verifies | First read and advance `next_validator_nonce`. If later validation fails, keep only that nonce change. If validation succeeds, insert Validator, insert or keep Cluster with `liquidated = false`, and insert one encrypted share record per operator id. |
| ValidatorRemoved | Validator exists for `(validator_public_key, owner)` and the event operator ids derive the validator's cluster id | Remove the Validator and its encrypted share records. If the cluster has no remaining validators, omit the Cluster. Then omit any removed Operator that no included cluster references. |
| ClusterLiquidated | Cluster exists for the event owner and operator ids | Set `liquidated = true` for that Cluster. |
| ClusterReactivated | Cluster exists for the event owner and operator ids | Set `liquidated = false` for that Cluster. |
| FeeRecipientAddressUpdated | Event decodes | Set the Owner fee recipient and leave `next_validator_nonce` unchanged. |
| ValidatorExited | Event decodes | No checkpoint state transition. |
...
- Client storage note. A client may store validator data under `validator_public_key` for local runtime behavior, but checkpoint materialization must recover the owner field and use the contract identity above. The root must not collapse two records that share a validator public key and have different owners.
